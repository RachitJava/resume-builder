...
public class ResumeParserService {
    ...
    private ResumeDTO parseWithAI(String resumeText, String apiKey) {
        ...
        String userPrompt = """
                ...
                - fullName (string)
                - jobTitle (string - professional title e.g. Software Engineer, Product Manager)
                - email (string)
                - phone (string)
                - location (string)
                - linkedIn (string)
    ...
    private ResumeDTO convertJsonToResumeDTO(String jsonContent) throws Exception {
        ...
        resume.setJobTitle(getTextOrNull(json, "jobTitle"));
    ...
    private void extractContactInfo(ResumeDTO resume, String content, List<String> lines) {
        ...
        Pattern phonePattern = Pattern
                .compile("\\+?\\d{1,3}[\\s.-]?\\(?\\d{3}\\)?[\\s.-]?\\d{3}[\\s.-]?\\d{4}|\\d{10,12}");
    ...
    private List<String> extractSkillsFromFullText(String content) {
        List<String> commonSkills = List.of(
                "Java", "Python", "JavaScript", "TypeScript", "C++", "C#", "Go", "Rust", "Ruby", "PHP", "Swift",
                "Kotlin",
        ...
        for (String skill : commonSkills) {
            if (Pattern.compile("\\b" + Pattern.quote(skill.toLowerCase()) + "\\b", Pattern.CASE_INSENSITIVE)
                    .matcher(contentLower).find()) {
